...
/**
 * Get the value of a specific attribute from a JSX element.
 *
 * @param {JSXElement} node - Node to get attribute from
 * @param {string} name - Name of the attribute
 * @returns {Expression|null} The value of the attribute if present
 */
exports.getAttributeValue = function(node, name) {
  var attr = exports.getAttributeMap(node)[name];
  if (!attr) return null;
  
  if (exports.isExpressionContainer(attr)) {
    return exports.getExpression(attr);
  } else if (exports.isStringLiteral(attr)) {
    return attr.value;
  }
  return null;
};
